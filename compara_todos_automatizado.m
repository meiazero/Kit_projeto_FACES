function compara_todos_automatizado()
  ...
  if info.bytes == 0
    header = ['modelo,dimensao,numero_treinamentos,tempo_execucao_em_segundos,' ...
              'media,minimo,maximo,mediana,desvio_padrao,r2_train,r2_test,com_pca,normalizacao,' ...
    ...
    for ia = 1:length(ativacoes)
      ...
      for io = 1:length(otimizadores)
        ...
        for ie = 1:length(etas)
          ...
          tempo = toc(t0);
          fprintf(fid, 'mlp1h,%s,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%s,%s,%s,%s,%.3f,%d\n', ...
            dimensao_str, Nr, tempo, STATS(1), STATS(2), STATS(3), STATS(4), STATS(5), R2_mlp1h_train, R2_mlp1h_test, com_pca, norm, act, opt, eta, epocas);
          ...
          tempo = toc(t0);
          fprintf(fid, 'mlp2h,%s,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%s,%s,%s,%s,%.3f,%d\n', ...
            dimensao_str, Nr, tempo, STATS(1), STATS(2), STATS(3), STATS(4), STATS(5), R2_mlp2h_train, R2_mlp2h_test, com_pca, norm, act, opt, eta, epocas);
        end
      end
    end
